...
const MODES = [
  ...
  },
  {
    ...
    tooltip: 'Generate SQL for review under governed query rules without executing it.',
  },
  {
    id: 'runsql',
    label: 'Run SQL',
    ...
    tooltip: 'Execute SQL only against authorized High Tech tables.',
  },
];

const SHOW_SQL_SAFETY_COPY = 'Generated SQL is shown for review and is not executed.';
const RUN_SQL_SAFETY_COPY = 'SQL is executed only against authorized High Tech views with governed access controls.';
const BLOCKED_QUERY_COPY = 'This query was not executed because it falls outside the allowed governed High Tech schema.';
...
const GOVERNED_QUERY_ERROR_PATTERNS = [
  ...
  /PostgreSQL syntax/i,
  /allowed governed High Tech schema/i,
];
...
const ASK_DATA_CONVERSATION_STORAGE_KEY = 'hightech.ask-data.conversation.v1';
const MAX_PERSISTED_CONVERSATION_MESSAGES = 24;

const RESULT_COLUMN_DISPLAY_LABELS = {
  ORDER_ID: 'CUSTOMER_COMMITMENT_ID',
  ORDER_STATUS: 'COMMITMENT_STATUS',
  ORDER_TOTAL: 'COMMITMENT_VALUE',
  CUSTOMER_ID: 'ENTERPRISE_BUYER_ID',
  CUSTOMER_NAME: 'ENTERPRISE_BUYER',
  CUSTOMER_CITY: 'BUYER_CITY',
  CUSTOMER_STATE: 'BUYER_REGION',
  SHIPPING_COST: 'LOGISTICS_COST',
  ITEMS: 'LINE_ITEMS',
  ITEM_ID: 'BOM_LINE_ID',
  PRODUCT_ID: 'PRODUCT_LIFECYCLE_ID',
  PRODUCT_NAME: 'HIGH_TECH_PRODUCT',
  UNIT_PRICE: 'UNIT_VALUE',
  LINE_TOTAL: 'COMMITMENT_LINE_VALUE',
  FULFILLMENT_CENTER: 'FAB_OR_CONTRACT_SITE',
  FULFILLMENT_CENTER_ID: 'FAB_OR_CONTRACT_SITE_ID',
...
const FALLBACK_SCHEMA_GROUPS = groupSchemaObjects(
  VISIBLE_SCHEMA_OBJECT_NAMES.map((objectName) => ({
    ...
    description: 'Queryable High Tech schema object.',
    example_questions: [],
...
function normalizeAskDataClientError(error) {
  ...
  const fallbackMessages = {
    API_UNREACHABLE: 'The Ask High Tech Data API is unreachable. Check that the app backend is running.',
    ...
    OLLAMA_MODEL_MISSING: 'The configured Ollama model is not available. Pull or configure the model before using Ask High Tech Data.',
    OLLAMA_TIMEOUT: 'The local Ollama service did not respond in time. Try again after the model finishes warming up.',
    SQL_GENERATION_FAILED: 'Unable to generate safe SQL for that question. Try a more specific product, commitment, fab, supplier, or signal.',
    SQL_VALIDATION_BLOCKED: BLOCKED_QUERY_COPY,
    ORACLE_QUERY_FAILED: 'Oracle could not execute the generated query. Try rephrasing with a more specific governed High Tech object.',
    REQUEST_TIMEOUT: 'The request took too long. Try a narrower High Tech question.',
    MALFORMED_LLM_RESPONSE: 'The model returned an unexpected response. Try again with a more specific High Tech data question.',
    UNEXPECTED_BACKEND_RESPONSE: 'Ask High Tech Data could not complete the request.',
  };
  ...
  return {
    ...
    detail: isBlocked && message !== BLOCKED_QUERY_COPY ? message : (error.detail || null),
...
function buildConversationHistory(messages) {
  return messages
    ...
    .map((message) => ({
      ...
      text: message.text || message.resultSummary || message.explanation || '',
    }))
...
function loadPersistedMessages() {
  ...
  try {
    const saved = JSON.parse(window.sessionStorage.getItem(ASK_DATA_CONVERSATION_STORAGE_KEY) || '[]');
    return Array.isArray(saved)
      ? saved.filter((message) => message && ['user', 'assistant'].includes(message.role)).slice(-MAX_PERSISTED_CONVERSATION_MESSAGES)
      : [];
  } catch (_) {
    return [];
  }
...
export default function AskData() {
  ...
  useEffect(() => {
    ...
    window.sessionStorage.setItem(ASK_DATA_CONVERSATION_STORAGE_KEY, JSON.stringify(persistedMessages));
  }, [messages]);
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      ...
      if (mode === 'narrate') {
        const result = await api.selectai.chat(question, true, profile, history);
        response = {
          role: 'assistant',
          mode: 'narrate',
          text: result.answer,
        ...
        const result = await api.selectai.chatMode(question, true, profile, history);
        response = {
          role: 'assistant',
          mode: 'chat',
          text: result.answer,
        ...
        const result = await api.selectai.showsql(question, profile, history);
        response = {
          role: 'assistant',
          mode: 'showsql',
        ...
        const result = await api.selectai.runsql(question, profile, history);
        response = {
          role: 'assistant',
          mode: 'runsql',
  ...
  const clearChat = useCallback(() => {
    ...
    if (typeof window !== 'undefined') {
      window.sessionStorage.removeItem(ASK_DATA_CONVERSATION_STORAGE_KEY);
    }
  }, []);
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {/* Oracle Internals */}
      <RegisterOraclePanel title="Ask Seer Tech Data">
...
-- Example question:
-- "What are the top high-tech products by product value?"
...
-- 4. The UI renders rows or a narrated answer`} />
      ...
      <div className="glass-card overflow-hidden" style={{ border: '1px solid rgba(79,125,123,0.25)' }}>
        ...
        <div className="px-5 py-2.5 space-y-1.5" style={{ background: 'var(--color-surface-muted)', borderBottom: '1px solid var(--color-border)' }}>
          ...
          <p className="text-[10px] text-[var(--color-text-dim)]">
            Choose how the assistant responds: explain the answer, continue a conversation, show generated SQL without running it, or run the query against governed High Tech tables.
          </p>
        </div>
          ...
          {messages.length === 0 && (
            <div className="space-y-4 py-6">
              <div className="text-center mb-4">
                ...
                <p className="text-sm text-[var(--color-text-dim)]">Ask about semiconductor manufacturing, fab operations, product lifecycle, supplier risk, customer commitments, field quality, warranty analytics, or support operations.</p>
              ...
              {/* Queryable High Tech schema */}
              <div className="askdata-schema-panel">
                <div className="askdata-schema-panel__header">
                  <div>
                    <p className="askdata-schema-panel__eyebrow">Queryable High Tech schema</p>
                ...
                <div className="askdata-schema-domain-pills" aria-label="High Tech schema domains">
                        ...
                        <div className="askdata-schema-object-list">
                          {group.objects.map((object) => (
                            <span
                              ...
                              title={`${object.display_name || object.object_name}: ${object.description || 'Queryable High Tech schema object.'}`}
                            >
                              {object.object_name}
                            </span>
                          ))}
        ...
        <div className="px-5 py-3" style={{ background: 'var(--color-surface-muted)', borderTop: '1px solid var(--color-border)' }}>
          <div className="jet-control-row">
            <div className="flex-1 min-w-[260px]" onKeyDown={handleKeyDown}>
              <JetInputText
                ...
                placeholder="Ask about fab operations, component shortages, channel inventory, customer commitments, field quality, warranty analytics, support operations, or NPI readiness..."
          ...
          {messages.length === 0 && !input.trim() && !sending && (
            <p className="mt-1.5 text-[10px] text-[var(--color-text-dim)]">
              Try one of the examples above or enter a question about the live High Tech schema.
            </p>
          )}
        </div>
      </div>
