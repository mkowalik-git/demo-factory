...
const MODES = [
  ...
  },
  {
    ...
    tooltip: 'Execute SQL only against authorized transportation views.',
  },
];
...
const RUN_SQL_SAFETY_COPY =
  'SQL is executed only against authorized transportation views with governed access controls.';
const BLOCKED_QUERY_COPY =
  'This query was not executed because it falls outside the allowed governed transportation schema.';
...
const GOVERNED_QUERY_ERROR_PATTERNS = [
  ...
  /PostgreSQL syntax/i,
  /allowed governed transportation schema/i,
];
...
const FALLBACK_SCHEMA_DOMAINS = {
  transport_orders_v: 'Shipment Orders',
  transport_services_v: 'Transport Services',
  service_lines_v: 'Transport Services',
  shippers_v: 'Shippers',
  shipper_signal_posts_v: 'Operational Signals',
  signal_sources_v: 'Operational Signals',
  logistics_terminals_v: 'Terminal Capacity',
  terminal_capacity_v: 'Terminal Capacity',
  transport_routes_v: 'Routes',
  demand_forecasts: 'Demand Forecasting',
  demand_regions: 'Demand Forecasting',
  fulfillment_zones: 'Terminal Capacity',
  transport_network_entities_v: 'Transport Network',
  transport_network_relationships_v: 'Transport Network',
  transport_exception_cases_v: 'Transport Network',
  event_stream: 'Operations Events',
...
const FALLBACK_SCHEMA_GROUPS = groupSchemaObjects(
  VISIBLE_SCHEMA_OBJECT_NAMES.map((objectName) => ({
    ...
    description: 'Queryable transportation schema object.',
    example_questions: [],
...
const RESULT_COLUMN_DISPLAY_LABELS = {
  ...
  ORDER_TOTAL: 'FREIGHT_VALUE',
  CUSTOMER_ID: 'SHIPPER_ID',
  CUSTOMER_NAME: 'SHIPPER_NAME',
  CUSTOMER_CITY: 'SHIPPER_CITY',
  CUSTOMER_STATE: 'SHIPPER_REGION',
  SHIPPING_COST: 'TRANSPORT_COST',
  ITEMS: 'SERVICE_LEGS',
  ITEM_ID: 'SERVICE_LEG_ID',
  PRODUCT_ID: 'TRANSPORT_SERVICE_ID',
  PRODUCT_NAME: 'TRANSPORT_SERVICE_NAME',
  BRAND_NAME: 'SERVICE_LINE_NAME',
  UNIT_PRICE: 'CONTRACT_RATE',
  LINE_TOTAL: 'SERVICE_LEG_VALUE',
  FULFILLMENT_CENTER: 'LOGISTICS_TERMINAL',
  FULFILLMENT_CENTER_ID: 'TERMINAL_ID',
...
function buildConversationHistory(messages) {
  return messages
    ...
    .map((message) => ({
      ...
      referencedData: message.referencedData || null,
    }))
...
export default function AskData() {
  ...
  return (
    <div className="space-y-6 fade-in">
      <TransportationStoryPanel scene="askdata" />

      {/* Oracle Internals */}
      <RegisterOraclePanel title="Ask Seer Transport Data">
        <div className="space-y-4">
          <div>
            <p className="text-xs font-semibold text-[var(--color-text-dim)] uppercase tracking-wider mb-2">What's Happening</p>
            <p className="text-[var(--color-text)] leading-relaxed">
              This page uses the app&apos;s natural-language SQL flow for the Welcome page.
...
-- EXPLAIN: draft SQL, execute it, summarize the operational answer
-- CHAT: draft SQL, execute it, return a conversational logistics explanation
-- SHOWSQL: inspect the generated SQL before execution
-- RUNSQL: execute the generated SQL and return raw rows

-- Example question:
-- "What are the top transportation services by service value?"

SELECT
      p.product_name AS transport_service,
      ...
      <div className="glass-card overflow-hidden" style={{ border: '1px solid rgba(79,125,123,0.25)' }}>
        ...
        <div className="px-5 py-3 flex items-center justify-between" style={{ background: 'rgba(79,125,123,0.06)', borderBottom: '1px solid rgba(79,125,123,0.15)' }}>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold">Seer Transport Data Assistant</h3>
              <p className="text-[10px] text-[var(--color-text-dim)]">
                Scenario questions translated into governed SQL over the live Seer Transport Network schema.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="px-5 py-2.5 space-y-1.5" style={{ background: 'var(--color-surface-muted)', borderBottom: '1px solid var(--color-border)' }}>
          ...
          <p className="text-[10px] text-[var(--color-text-dim)]">
            Choose how the assistant responds: explain the operational answer, continue a conversation, show generated SQL, or run the query against governed transportation views.
          </p>
        </div>
          ...
          {messages.length === 0 && (
            <div className="space-y-4 py-6">
              <div className="text-center mb-4">
                ...
                <p className="text-sm text-[var(--color-text-dim)]">Ask about transport services, terminal capacity, operational signals, routes, shipment orders, or exception cases.</p>
              ...
              {/* Queryable transportation schema */}
              <div className="askdata-schema-panel">
                <div className="askdata-schema-panel__header">
                  <div>
                    <p className="askdata-schema-panel__eyebrow">Queryable transportation schema</p>
                ...
                <div className="askdata-schema-domain-pills" aria-label="Transportation schema domains">
                        ...
                        <div className="askdata-schema-object-list">
                          {group.objects.map((object) => (
                            <span
                              ...
                              title={`${object.display_name || object.object_name}: ${object.description || 'Queryable transportation schema object.'}`}
                            >
                              {object.object_name}
                            </span>
                          ))}
        ...
        <div className="px-5 py-3" style={{ background: 'var(--color-surface-muted)', borderTop: '1px solid var(--color-border)' }}>
          <div className="jet-control-row">
            <div className="flex-1 min-w-[260px]" onKeyDown={handleKeyDown}>
              ...
              placeholder="Ask about lane risk, terminal capacity, shipment exceptions, or freight value at risk..."
          ...
          {messages.length === 0 && !input.trim() && !sending && (
            <p className="mt-1.5 text-[10px] text-[var(--color-text-dim)]">
              Try one of the examples above or enter a question about the live transportation schema.
            </p>
          )}
        </div>
      </div>
