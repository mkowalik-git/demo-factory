...
const MODES = [
  ...
  },
  {
    ...
    tooltip: 'Execute SQL only against authorized media views.',
  },
];
...
const RUN_SQL_SAFETY_COPY =
  'SQL is executed only against authorized media views with governed access controls.';
const BLOCKED_QUERY_COPY =
  'This query was not executed because it falls outside the allowed governed media schema.';
...
const GOVERNED_QUERY_ERROR_PATTERNS = [
  ...
  /PostgreSQL syntax/i,
  /allowed governed media schema/i,
];
...
const FALLBACK_SCHEMA_DOMAINS = {
  media_campaign_orders_v: 'Campaign Requests',
  orders: 'Campaign Requests',
  order_items: 'Campaign Requests',
  media_content_assets_v: 'Content Assets',
  products: 'Content Assets',
  brands: 'Content Assets',
  media_audience_signals_v: 'Audience Signals',
  social_posts: 'Audience Signals',
  customers: 'Audience Segments',
  media_distribution_capacity_v: 'Distribution & Capacity',
  fulfillment_centers: 'Distribution & Capacity',
  inventory: 'Distribution & Capacity',
  demand_forecasts: 'Distribution & Capacity',
  demand_regions: 'Distribution & Capacity',
  shipments: 'Distribution & Capacity',
  media_creator_relationships_v: 'Creator Graph',
  influencers: 'Creator Graph',
...
const FALLBACK_SCHEMA_GROUPS = groupSchemaObjects(
  VISIBLE_SCHEMA_OBJECT_NAMES.map((objectName) => ({
    ...
    description: 'Queryable media schema object.',
    example_questions: [],
...
const RESULT_COLUMN_DISPLAY_LABELS = {
  ORDER_ID: 'CAMPAIGN_ORDER_ID',
  ORDER_STATUS: 'CAMPAIGN_STATUS',
  ORDER_TOTAL: 'CAMPAIGN_VALUE',
  CUSTOMER_ID: 'AUDIENCE_ACCOUNT_ID',
  CUSTOMER_NAME: 'AUDIENCE_ACCOUNT',
  CUSTOMER_CITY: 'AUDIENCE_CITY',
  CUSTOMER_STATE: 'AUDIENCE_REGION',
  SHIPPING_COST: 'DISTRIBUTION_COST',
  ITEMS: 'LINE_ITEMS',
  ITEM_ID: 'LINE_ITEM_ID',
  PRODUCT_ID: 'CONTENT_ASSET_ID',
  PRODUCT_NAME: 'CONTENT_ASSET',
  UNIT_PRICE: 'CAMPAIGN_VALUE_PROXY',
  LINE_TOTAL: 'CAMPAIGN_LINE_VALUE',
  FULFILLMENT_CENTER: 'DISTRIBUTION_HUB',
  FULFILLMENT_CENTER_ID: 'DISTRIBUTION_HUB_ID',
  SOCIAL_DRIVEN: 'AUDIENCE_SIGNAL_INFLUENCED_FLAG',
};
...
function normalizeAskDataClientError(error) {
  ...
  const fallbackMessages = {
    API_UNREACHABLE: 'The Ask Media and Entertainment Data API is unreachable. Check that the app backend is running.',
    ...
    OLLAMA_MODEL_MISSING: 'Model llama3.2 is not available in Ollama. Pull or configure the model before using Ask Media and Entertainment Data.',
    ...
    ORACLE_QUERY_FAILED: 'Oracle could not execute the generated query. Try rephrasing with a more specific governed media view.',
    ...
    UNEXPECTED_BACKEND_RESPONSE: 'Ask Media and Entertainment Data could not complete the request.',
  };
...
export default function AskData() {
  ...
  const sendMessage = useCallback(async (text) => {
    ...
    try {
      let response;
      if (mode === 'narrate') {
        const result = await api.selectai.chat(question, true, profile, conversationHistory);
        response = {
          role: 'assistant',
          mode: 'narrate',
          text: result.answer,
        ...
        const result = await api.selectai.chatMode(question, true, profile, conversationHistory);
        response = {
          role: 'assistant',
          mode: 'chat',
          text: result.answer,
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {/* Oracle Internals */}
      <RegisterOraclePanel title="Ask Media and Entertainment Data">
...
-- Example question:
-- "Which audience segments have the highest campaign value?"

SELECT
  audience_tier,
  COUNT(campaign_order_id) AS campaign_requests,
...
-- 4. The UI renders rows or a narrated answer`} />
      ...
      <div>
        <h2 className="text-2xl font-bold flex items-center gap-2">
          <JetGlyph iconClass="oj-fwk-icon-message-info" className="askdata-page-glyph tone-teal" /> Ask Media and Entertainment Data
        </h2>
        <p className="text-sm text-[var(--color-text-dim)] mt-1">
          Ask questions about viewers, subscribers, fans, content assets, creator performance, campaign requests, live events, moderation signals, distribution capacity, and churn or engagement risk. The assistant can explain results, show generated SQL, or execute governed queries against the live media schema.
        </p>
      </div>
      ...
      <div className="glass-card overflow-hidden" style={{ border: '1px solid rgba(79,125,123,0.25)' }}>
        ...
        <div className="px-5 py-3 flex items-center justify-between" style={{ background: 'rgba(79,125,123,0.06)', borderBottom: '1px solid rgba(79,125,123,0.15)' }}>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold">Media and Entertainment Data Assistant</h3>
              <p className="text-[10px] text-[var(--color-text-dim)]">
                Natural-language questions translated into governed SQL over the live Seer Media schema.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
        ...
        <div className="px-5 py-2.5 space-y-1.5" style={{ background: 'var(--color-surface-muted)', borderBottom: '1px solid var(--color-border)' }}>
          ...
          <p className="text-[10px] text-[var(--color-text-dim)]">
            Choose how the assistant responds: explain the answer, continue a conversation, show generated SQL, or run the query against governed media views.
          </p>
        </div>
          ...
          {messages.length === 0 && (
            <div className="space-y-4 py-6">
              <div className="text-center mb-4">
                ...
                <p className="text-sm text-[var(--color-text-dim)]">Ask about subscribers, fans, creators, streams, campaigns, content releases, premiere events, content revenue, audience signals, or distribution capacity.</p>
              ...
              {/* Queryable media schema */}
              <div className="askdata-schema-panel">
                <div className="askdata-schema-panel__header">
                  <div>
                    <p className="askdata-schema-panel__eyebrow">Queryable media schema</p>
                ...
                <div className="askdata-schema-domain-pills" aria-label="Media schema domains">
                        ...
                        <div className="askdata-schema-object-list">
                          {group.objects.map((object) => (
                            <span
                              ...
                              title={`${object.display_name || object.object_name}: ${object.description || 'Queryable media schema object.'}`}
                            >
                              {object.object_name}
                            </span>
                          ))}
        ...
        <div className="px-5 py-3" style={{ background: 'var(--color-surface-muted)', borderTop: '1px solid var(--color-border)' }}>
          <div className="jet-control-row">
            <div className="flex-1 min-w-[260px]" onKeyDown={handleKeyDown}>
              <JetInputText
                ...
                placeholder="Ask about subscriber churn, creator performance, campaign requests, audience signals, content recommendations, or live event capacity..."
          ...
          {messages.length === 0 && !input.trim() && !sending && (
            <p className="mt-1.5 text-[10px] text-[var(--color-text-dim)]">
              Try one of the examples above or enter a question about the live media schema.
            </p>
          )}
        </div>
      </div>
