...
const MODES = [
  ...
  },
  {
    ...
    tooltip: 'Execute SQL only against authorized Life Sciences data objects.',
  },
];
...
const RUN_SQL_SAFETY_COPY =
  'SQL is executed only against authorized Life Sciences objects with governed access controls.';
const BLOCKED_QUERY_COPY =
  'This query was not executed because it falls outside the allowed governed Life Sciences schema.';
...
const EXAMPLE_QUESTIONS = [
  { text: 'Which quality signals create the highest clinical supply value at risk?', category: 'Signals' },
  { text: 'Show patient-dose supply exposure by regulated product category.', category: 'Supply Exposure' },
  { text: 'Which manufacturers are linked to critical quality signals?', category: 'Network' },
  { text: 'Which Seer cold-chain sites have the most available controlled inventory?', category: 'Cold Chain' },
  { text: 'Which trial sites have the highest clinical supply order value?', category: 'Trial Sites' },
  { text: 'Show supply exposure by product category and trial-site criticality.', category: 'Trial Sites' },
  { text: 'What is the total value of signal-linked clinical supply orders?', category: 'Orders' },
  { text: 'Which regulated products share quality signals with cold-chain categories?', category: 'Signals' },
];
...
const FALLBACK_SCHEMA_DOMAINS = {
  ls_clinical_supply_orders_v: 'Clinical Supply Orders',
  ls_regulated_products_v: 'Products & Capacity',
  ls_trial_sites_v: 'Clinical Supply Orders',
  ls_quality_signals_v: 'Quality Signals',
  ls_signal_sources_v: 'Quality Signals',
  ls_manufacturers_v: 'Products & Capacity',
  ls_cold_chain_sites_v: 'Cold Chain Logistics',
  ls_supply_capacity_v: 'Products & Capacity',
  ls_cold_chain_routes_v: 'Cold Chain Logistics',
  ls_operations_dashboard_v: 'Clinical Supply Orders',
};
...
const FALLBACK_PROFILES = [
  {
    ...
    desc: 'Ollama-served governed SQL model',
  },
];
...
function normalizeAskDataClientError(error) {
  ...
  const fallbackMessages = {
    API_UNREACHABLE: 'The Ask Seer Regulated Supply Data API is unreachable. Check that the app backend is running.',
    OLLAMA_UNAVAILABLE: 'The Ollama model service is unavailable. Check that Ollama is running.',
    OLLAMA_MODEL_MISSING: 'The configured Ollama model is not available. Pull or configure it before using Ask Seer Regulated Supply Data.',
...
function NarrativeAnswer({ msg, tone = 'teal', onFollowUp, label }) {
  ...
  return (
    <div className={`askdata-answer-card askdata-answer-card--${tone}`}>
      {label && <p className="askdata-answer-card__label">{label}</p>}
      ...
      {msg.resultSummary && <p className="askdata-result-summary">{msg.resultSummary}</p>}
...
export default function AskData() {
  ...
  useEffect(() => {
    ...
    api.selectai.schemaObjects().then((data) => {
      const apiGroups = Array.isArray(data.domains)
        ? data.domains
          .map((group) => ({
            domain: group.domain,
            objects: (group.objects || []).filter((object) => object.is_queryable_by_assistant !== false),
            object_count: (group.objects || []).filter((object) => object.is_queryable_by_assistant !== false).length,
          }))
          .filter((group) => group.objects.length > 0)
        : [];

      const flatObjects = Array.isArray(data.objects)
        ? data.objects.filter((object) => object.is_queryable_by_assistant !== false)
        : [];

      const nextGroups = apiGroups.length > 0 ? apiGroups : groupSchemaObjects(flatObjects);
  ...
  return (
    <div className="space-y-6 fade-in">
      ...
      {/* Oracle Internals */}
      <RegisterOraclePanel title="Ask Seer Regulated Supply Data">
        <div className="space-y-4">
          <div>
            ...
            <p className="text-[var(--color-text)] leading-relaxed">
              ...
              Your question is sent to the selected <span className="tone-plum font-mono">Ollama model</span> with schema context and the selected profile,
              ...
              Oracle AI Database 26ai remains the governed data access and SQL execution layer for this connected demo schema; the language model runtime is external to the database.
            </p>
          </div>
          <div className="flex flex-wrap gap-1.5">
            <FeatureBadge label="Ollama" color="purple" />
          ...
          <SqlBlock code={`-- Ask Data runtime: question -> governed SQL -> Oracle SQL -> UI answer
...
-- Example question:
-- "Show patient-dose supply exposure by regulated product category."

SELECT
  p.product_name,
  ROUND(SUM(oi.quantity * oi.unit_price), 2) AS supply_value
FROM order_items oi
...
-- Behind the scenes:
-- 1. The app sends the question + schema hints to the configured Ollama model
-- 2. The Ollama model drafts SQL for the selected mode
...
-- 4. The UI renders rows or a narrated answer`} />
          ...
          <div>
            ...
            <div className="space-y-1" style={{ fontSize: 9 }}>
              ...
              <DiagramBox label="App builds prompt + schema context" sub="Includes the selected Ollama model profile" color="#796087" />
              <div className="text-center text-[var(--color-text-dim)]">↓</div>
          ...
          <div className="rounded-lg p-2 text-[9px]" style={{ background: 'rgba(79,125,123,0.08)', border: '1px dashed rgba(79,125,123,0.3)', color: 'var(--color-text)' }}>
            <span className="font-semibold">Key insight:</span> The language runtime handles reasoning,
            while Oracle AI Database 26ai provides governed query execution and result retrieval for the connected intelligence layer.
            Source systems continue to originate the clinical, quality, and logistics transactions. This page shows the generated SQL so you can inspect what runs against the schema.
          </div>
        </div>
      </RegisterOraclePanel>
      ...
      <div className="glass-card overflow-hidden" style={{ border: '1px solid rgba(79,125,123,0.25)' }}>
        ...
        <div className="px-5 py-3 flex items-center justify-between" style={{ background: 'rgba(79,125,123,0.06)', borderBottom: '1px solid rgba(79,125,123,0.15)' }}>
          <div className="flex items-center gap-2">
            ...
            <div>
              <h3 className="text-sm font-bold">Ask Seer Regulated Supply Data Chat</h3>
              <p className="text-[10px] text-[var(--color-text-dim)]">
                Natural-language questions translated into governed SQL over the live Seer clinical-supply schema.
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <div className="askdata-profile-select">
              ...
              <p className="askdata-profile-select__meta">
                {activeProfile?.desc || 'Ollama model profile'} · {activeProfile?.provider || 'Ollama + Oracle SQL'}
              </p>
            </div>
            {messages.length > 0 && (
              <JetButton
        ...
        <div className="px-5 py-2.5 space-y-1.5" style={{ background: 'var(--color-surface-muted)', borderBottom: '1px solid var(--color-border)' }}>
          ...
          <p className="text-[10px] text-[var(--color-text-dim)]">
            Choose how the assistant responds: explain the answer, continue a conversation, show generated SQL, or run the query against governed Life Sciences data objects.
          </p>
        </div>
          ...
          {messages.length === 0 && (
            <div className="space-y-4 py-6">
              <div className="text-center mb-4">
                ...
                <p className="text-sm text-[var(--color-text-dim)]">Ask about regulated products, trial sites, cold-chain capacity, quality signals, manufacturers, or clinical supply orders.</p>
              ...
              {/* Queryable Life Sciences schema */}
              <div className="askdata-schema-panel">
                <div className="askdata-schema-panel__header">
                  <div>
                    <p className="askdata-schema-panel__eyebrow">Queryable Life Sciences schema</p>
                    <p className="askdata-schema-panel__copy">
                      {schemaGroups.length} domains · {schemaObjectCount} queryable semantic objects · SQL names available for inspection.
                    </p>
                  </div>
                ...
                <div className="askdata-schema-domain-pills" aria-label="Life Sciences schema domains">
                        ...
                        <div className="askdata-schema-object-list">
                          {group.objects.map((object) => (
                            <span
                              ...
                              title={`${object.display_name || object.object_name}: ${object.description || 'Queryable Life Sciences schema object.'}`}
                            >
                              {object.object_name}
                            </span>
                          ))}
              ...
              <div className={`max-w-[85%] ${msg.role === 'user' ? '' : 'w-full'}`}>
                ...
                {msg.role === 'assistant' && (
                  <div className="space-y-2">
                    ...
                    {msg.mode === 'narrate' && (
                      <>
                        <NarrativeAnswer msg={msg} tone="teal" label="Evidence-based explanation" onFollowUp={sendMessage} />
                    ...
                    {msg.mode === 'chat' && (
                      <>
                        <NarrativeAnswer msg={msg} tone="ocean" label="Conversation reply" onFollowUp={sendMessage} />
                        <GeneratedSqlDetails sql={msg.sql} />
                      </>
                    )}
          ...
          <div className="jet-control-row">
            <div className="flex-1 min-w-[260px]" onKeyDown={handleKeyDown}>
              <JetInputText
                ...
                placeholder="Ask about regulated products, trial sites, cold-chain capacity, quality signals, manufacturers, or clinical supply orders..."
